...
gen_screenshots() {
  ...
  [[ -s "$screen_file" ]] && upload_image_com "$screen_file"  # static.sh
  [[ $enable_byrbt == yes && -s "$screen_file" ]] && upload_image_byrbt "$screen_file"
  # delete image if upload successed
  [[ "$img_url_com" || "$img_url_byr" ]] && sleep 0.5 && \rm -f "$screen_file"
}
...
generate_info_local() {
  ...
  # 缩略图
  unset img_url_com img_url_byr # clean
  gen_screenshots
  # 存档
  [[ "$img_url_com" ]] && \
