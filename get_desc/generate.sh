...
get_douban_url_by_keywords() {
    ...
    if [ "$(echo "$dot_name"|grep -Eo '[12][0198][0-9]{2}')" ]; then
        local search_base_name="$(echo "$dot_name"|grep -Eo '.*[12][0198][0-9]{2}')"
    # 剧集单集
    elif [ "$(echo "$dot_name"|sed -r 's/.*\.(S[0-9]+)?([-]?Ep?[0-9]+)?\..*/\1/i')" ]; then
        local search_base_name="$(echo "$dot_name"|sed -r 's/(.*)\.(S[0-9]+)?([-]?Ep?[0-9]+)?\..*/\1/i')"
    # 剧集单集,可能误判
    elif [ "$(echo "$dot_name"|sed -r 's/.*\.([-sEp0-9]+)\..*/\1/i')" ]; then
        local search_base_name="$(echo "$dot_name"|sed -r 's/(.*)\.([-sEp0-9]+)\..*/\1/i')"
    # 剧集合集
