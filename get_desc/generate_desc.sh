...
search_keyword_get_douban_url() {
    local base_movie_name_search="$(echo "$dot_name" |egrep -o '.*[12][098][0-9]{2}')"
    local search_get_douban_url="$(http --pretty=format --ignore-stdin GET "https://api.douban.com/v2/movie/search?q=${base_movie_name_search}"|grep 'movie.douban.com/subject'|head -1|awk -F '"' '{print $4}')"
...
from_douban_get_desc() {
    ...
    if [ "$USE_Local_Gen" = 'yes' ]; then
        local desc_json_info="$("$python3" -c "import sys;sys.path.append(\"${AUTO_ROOT_PATH}/get_desc/\");from gen import Gen;import json;gen=Gen(\"${search_desc_url}\").gen(_debug=True);print(json.dumps(gen,sort_keys=True,indent=2,separators=(',',':'),ensure_ascii=False))")"
    else
        local desc_json_info="$(http --pretty=format --ignore-stdin GET "https://api.rhilip.info/tool/movieinfo/gen?url=${search_desc_url}")"
    fi
    ...
    [ "$enable_byrbt" = 'yes' ] && gen_desc_html="$(echo "$gen_desc_bbcode"|sed 's#$#&<br />#g')"
}

poster_up_to_sm_and_byr() {
    local tmp_poster_file="$AUTO_ROOT_PATH/tmp/$(echo $RANDOM)-$(echo $RANDOM)-${douban_poster_url##*/}"
    ...
    unset tmp_poster_file douban_poster_url
}
...
generate_main_func() {
    ...
    local source_detail_desc_tmp="$(echo -e "[img]${new_poster_url}[/img]\n${gen_desc_bbcode}\n")
    ...
    [ "$enable_byrbt" = 'yes' ] && local source_detail_html_tmp="$(echo -e "<img src=\"${new_poster_url_byrbt}\" /><br />\n${gen_desc_html}<br />\n\n")
    ...
    [ "$enable_byrbt" = 'yes' ] && echo "$source_detail_html_tmp" > "$source_detail_html"
}
