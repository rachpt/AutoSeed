...
if [ -z "$TR_TORRENT_NAME" ]; then
    ...
    do
        oneTorrent=`"$trans_remote" ${HOST}:${PORT} --auth ${USER}:${PASSWORD} -t $oneTorrentID -i |grep 'Name'|head -n 1|sed 's/  Name: //ig'`
...
function main_loop()
{
    ...
    do
   	    new_torrent_name=`$trans_show "${flexget_path}/$i"|grep 'Name'|head -n 1|sed 's/Name: //i;s/\.mkv$//i;s/\.ts$//i;s/\.mp4$//i;s/\.avi$//i'`
        if [ "$new_torrent_name" != "$(echo "$new_torrent_name"|grep -oP '[-\.a-z0-9A-Z@_ ]+')" ]; then
            #---special for non-standard 0day-name---#
            new_torrent_name="$($trans_show "${flexget_path}/$i"|grep -A 10
            'FILES'|egrep -i '[\.0-9]+[ ]*(GB|MB)'|egrep -io '[-\.a-z0-9@ ]+'|tail -n 2|head -n 1|sed 's/^[\. ]\+//;s/\.[a-z4 ]\{3,5\}$//i'|sed 's/\.sample//i')"
        fi
        ...
        dot_name="$(echo "$new_torrent_name"|sed "s/[ ]\+/./g;s/\(.*\)\.mp4/\1/gi;s/\(.*\)\.mkv/\1/gi")"
